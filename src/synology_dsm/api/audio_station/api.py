...
from .models.audio_station_info import AudioStationInfo
...
class AudioStationApi:
    ...
    def get_info(self) -> Optional[AudioStationInfo]:
        opts = {
            "version": 4,
        }

        res = self._dsm.get(self.INFO_API_KEY, "getinfo", data=opts)
        if res["success"]:
            return AudioStationInfo(res["data"])
        return None

    def remote_player_get_current_playlist(self, player_uuid: str) -> Optional[Playlist]:
    ...
    def remote_player_get_player_status(self, player_uuid: str) -> Optional[RemotePlayerStatus]:
    ...
    def remote_player_get_players(self) -> List[Player]:
        ...
        if res["success"]:
            return PlayerList(res["data"]).players
        return []
